...
上傳修復文件至/patch目錄，重命名為patched，然後通過創建version文件來確認是否進行patch，等待30s左右環境會自動進行替換同時清空patch目錄，替換後的文件可在/challenge下查看

### 修復說明

1. 應當針對漏洞點附近的代碼段進行修復，與漏洞點無關函數不允許修改，同時應保持賽題服務正常以及交互邏輯不變
