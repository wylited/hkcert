...
def get_our_ip(creds_file=None):
    ...
    Args:
        creds_file: Path to CSV/XLSX/XLS file (optional)
    ...
    # Handle XLS files
    if creds_file.endswith('.xls'):
        try:
            import xlrd
            wb = xlrd.open_workbook(creds_file)
            ws = wb.sheet_by_index(0)
            # Our IP is typically in row 1 (after "Login Info" header)
            for i in range(min(5, ws.nrows)):
                if ws.cell_value(i, 0) == 'IP':
                    ip = str(ws.cell_value(i, 1))
                    if _is_valid_ip(ip):
                        return ip
            return None
        except ImportError:
