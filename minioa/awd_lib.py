...
API_URL = "http://10.2.60.1/api/ct/web/awd_race/race/38d769e349e4ee6e6d25aa4a2d0906f1/flag/robot/"
TOKEN = "9b9b71ea9cc34567687378baa78eb6d3"

# Path to credentials file (CSV or XLSX)
CREDS_FILE = "minioa.csv"  # or "mqda.xlsx"

# PEM file settings
PEM_URL = "http://10.2.60.1:80/ct/upload/other/pvt-ctf-2c6621788438bcb98a757321432bb1e9.pem"
PEM_FILE = "auth.pem"
...
def _parse_csv(filepath):
    ...
    our_ip = None
    in_other_teams = False
    
    for i, line in enumerate(lines):
        line = line.strip()
        
        # Detect "Other Team Info" section
        if 'Other Team Info' in line:
            in_other_teams = True
            continue
        
        # Get our IP (before Other Team Info section)
        if line.startswith('IP,') and not in_other_teams:
            ip = line.split(',')[1].strip()
            if _is_valid_ip(ip):
                our_ip = ip
            continue
        
        # Get target IPs (after Other Team Info section)
        if line.startswith('IP,') and in_other_teams:
            ip = line.split(',')[1].strip()
            if _is_valid_ip(ip):
                targets.append(ip)
            continue
        
        # Also handle Guest_Node format (old format)
        if 'Guest_Node' in line:
            if i + 1 < len(lines):
