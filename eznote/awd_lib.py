...
CREDS_FILE = "eznote.csv"  # or "mqda.xlsx"
...
def _parse_csv(filepath):
    ...
    # First pass: find our IP (in Login Info section, before "Other Team Info")
    in_login_section = False
    in_other_teams = False
    
    for i, line in enumerate(lines):
        line = line.strip()
        if 'Login Info' in line:
        ...
        if line.startswith('IP,'):
            ...
            if ip and _is_valid_ip(ip):
                if in_login_section and our_ip is None:
                    our_ip = ip
                elif in_other_teams:
                    targets.append(ip)
