...
注：修復前請備份賽題附件到本地，否則修復後原始附件會被覆蓋！

## 選手環境

選手僅可透過sftp上傳修復文件到patch目錄，關鍵目錄功能如下：
...
上傳修復文件至/patch目錄，重命名為patched，然後通過創建version文件來確認是否進行patch，等待15s左右環境會自動進行替換同時清空patch目錄，替換後的文件可在/challenge下查看。

## Patch 說明

1. 應當針對漏洞點進行修復，與漏洞點無關函數不允許修改，同時應保持賽題服務正常以及交互邏輯不變
2. 須保持賽題文件大小不變，且允許修改最大位元組數為10位元組
